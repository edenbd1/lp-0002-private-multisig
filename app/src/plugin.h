...
// IComponent (offset-to-top, typeinfo, the two destructors, then createWidget
// and destroyWidget). An extra virtual here — `name()`, say — shifts every
// later slot, so the host would call the wrong function through a correctly
// cast pointer. Verified against LogosBasecamp 0.2.2.
class IComponent {
public:
...
public:
    ...
    QString  name() const { return QStringLiteral("lp_0002_multisig"); }
