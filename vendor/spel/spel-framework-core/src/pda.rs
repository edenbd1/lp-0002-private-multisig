...
pub fn compute_private_pda(
    ...
    let pda_seed = PdaSeed::new(combined);
    // LEZ folds a u128 `identifier` and, since v0.2.4, a ViewingPublicKey
    // (ML-KEM 768) into private-PDA derivation. The identifier defaults to 0
    // (the common case) until callers need to pass a specific one.
