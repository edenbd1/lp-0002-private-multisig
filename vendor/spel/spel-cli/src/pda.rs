...
fn resolve_seed(
    ...
    match seed {
        ...
        IdlSeed::Account { path } => {
            let account_id = account_map.get(path).ok_or_else(|| {
                format!(
        ...
        IdlSeed::Arg { path } => {
            let val = parsed_args.get(path).ok_or_else(|| {
                format!("PDA seed references arg '{}' which wasn't provided", path)
            })?;
...
pub fn compute_pda_from_seeds(
    ...
    if let Some(_npk) = npk {
        // LEZ v0.2.4 folds a ViewingPublicKey (ML-KEM 768) into private-PDA
        // derivation. spel has no way to supply one yet, and guessing would
        // silently produce a wrong address, so refuse rather than mislead.
        Err("private PDA derivation needs a viewing key on LEZ >= v0.2.4; \
